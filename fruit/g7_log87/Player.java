...
public class Player extends fruit.sim.Player
{
	...
	public double offset(int round, double expectedValue){
		...
		double NORMAL_MEDIAN = 0;
		double NORMAL_SD = totBowls/3;
		double NORMAL_CONSTANT = 10;

		if(totBowls == 0)
			return 0;
		
		
		double logReturn = LOG_CONSTANT * expectedValue * Math.log(bowlsLeft) - expectedValue;
		System.out.println("logReturn:"+logReturn+"\n");
		return logReturn;
		
		//}
		/*else if(false){
		
			double normalFunction = (1/(NORMAL_SD*Math.sqrt(2*3.1415)))*Math.pow(2.7183,-.5*(bowlsSeen[round]-NORMAL_MEDIAN)*(bowlsSeen[round]-NORMAL_MEDIAN)/(NORMAL_SD*NORMAL_SD));
			double normalReturn = normalFunction * expectedValue * NORMAL_CONSTANT - expectedValue;
			System.out.println("normalFunction"+normalFunction+"\n");
			System.out.println("expectedValue"+expectedValue+"\n");
			System.out.println("normalReturn:"+(normalReturn+expectedValue)+"\n");
			return normalReturn;
